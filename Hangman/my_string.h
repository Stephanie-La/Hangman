...
typedef enum status {FAILURE = 0, SUCCESS = 1}Status;
typedef enum boolean {FALSE, TRUE}Boolean;
//Precondition: None
//Postcondition: Allocate space for a string object that represesnts the empty
// string. The string will have capacity 7 and size 0 by default. A copy of
//the address of the opaque object will return on success and NULL on
//failure.


MY_STRING my_string_init_default(void);//with the same value as the given c-string. The capacity of the string
...
//the system and te handle referred to by the pointer phMy_string has been
//set to NULL.
...
//copy of the address of the opaque object will be returned success and
 //NULL on failure.
...
//Postcondition: returns an integer less than zero if the string repeated
...
//Postcondition: hMy_string will be the handle of a string object.
...
//necessariy resize the array unless needed.
...
//the cost of resize by making the string capacity somewhat larger than
...
//Guaranteed to not cause the resize operation of the internal data. Returns
...
//index if the index is in bounds but otherwise return NULL. This address
...
// attempt to resize and hold the data in right. On failure pLeft will be
// left as NULL and any memory that may ave been used by a potential
// object indicated by pLeft will be rturned to the freestore.
void my_string_assignment(Item* pLeft, Item Right);

//Precondition: current_word_family, new_key and word are all handles to valid
// MY_STRING opaque objects.  uess is an alphabetical character that can be either
// upper or lower case.
